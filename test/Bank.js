...
// daiWhaleBalance = daiContract.methods
//     .balanceOf(daiWhale)
//     .call()
//     .then(function (value) {
//         console.log('Balance of daiWhale: ' + value);
//     });
...
contract('Bank', function (accounts) {
    ...
    it('depositToBank', async () => {
        ...
        expect(
            new BN(await daiContract.methods.balanceOf(bankUser).call())
        ).to.be.bignumber.at.least(new BN(ether('1000')));
        ...
        let bankBalanceBeforeDeposit  =
            await bankInstance.getBalanceForBankUser(bankUser);
        let bankUserDaiBalanceBeforeDeposit = new BN(
            await daiContract.methods.balanceOf(bankUser).call()
        );

        console.log(
            `bankBalanceBeforeDeposit: ${bankBalanceBeforeDeposit} bankUserDaiBalanceBeforeDeposit: ${bankUserDaiBalanceBeforeDeposit}`
        );
        // Call the deposit function within bankInstance

        await bankInstance.deposit(amountToDeposit.toString(), {
            from: bankUser,
            gasLimit: 800000,
        });

        let bankBalanceAfterDeposit =
            await bankInstance.getBalanceForBankUser(bankUser);
        let bankUserDaiBalanceAfterDeposit = new BN(
            await daiContract.methods.balanceOf(bankUser).call()
        );

        console.log(
            `bankBalanceAfterDeposit: ${bankBalanceAfterDeposit} bankUserDaiBalanceAfterDeposit: ${bankUserDaiBalanceAfterDeposit} amountToDeposit: ${amountToDeposit}`
        );

        // Check both the DAI balances and the bank balances to ensure that the bank
        // properly received the DAI
        expect(bankBalanceAfterDeposit).to.be.bignumber.to.equal(
            bankBalanceBeforeDeposit.add(amountToDeposit)
        );
        expect(bankUserDaiBalanceAfterDeposit).to.be.bignumber.to.equal(
            bankUserDaiBalanceBeforeDeposit.sub(amountToDeposit)
        );
    });
    ...
    it('withdrawFromBank', async () => {
        ...
        let fee = 0;
        await bankInstance.setBankFee(fee, {
            from: bankOwner,
            gasLimit: 800000,
        });
        ...
        let bankBalanceBeforeWithdrawal =
            await bankInstance.getBalanceForBankUser(bankUser);
        let bankUserDaiBalanceBeforeWithdrawal = new BN(
            await daiContract.methods.balanceOf(bankUser).call()
        );
        let bankFeeBalanceBeforeWithdrawal = new BN(
            await daiContract.methods.balanceOf(bankFeeAddress).call()
        );
        console.log(
            `bankBalanceBeforeWithdrawal: ${bankBalanceBeforeWithdrawal} bankUserDaiBalanceBeforeWithdrawal: ${bankUserDaiBalanceBeforeWithdrawal} bankFeeBalanceBeforeWithdrawal: ${bankFeeBalanceBeforeWithdrawal}`
        );

        // Call the withdraw function within bankInstance
        await bankInstance.withdraw(amountToWithdraw.toString(), {
            from: bankUser,
            gasLimit: 800000,
        });

        let bankBalanceAfterWithdrawal =
            await bankInstance.getBalanceForBankUser(bankUser);
        let bankUserDaiBalanceAfterWithdrawal = new BN(
            await daiContract.methods.balanceOf(bankUser).call()
        );
        let bankFeeBalanceAfterWithdrawal = new BN(
            await daiContract.methods.balanceOf(bankFeeAddress).call()
        );
        console.log(
            `bankBalanceAfterWithdrawal: ${bankBalanceAfterWithdrawal} bankUserDaiBalanceAfterWithdrawal: ${bankUserDaiBalanceAfterWithdrawal} bankFeeBalanceAfterWithdrawal: ${bankFeeBalanceAfterWithdrawal} amountToWithdraw:${`amountToWithdraw`}`
        );
        ...
        expect(bankBalanceAfterWithdrawal).to.be.bignumber.to.equal(
            bankBalanceBeforeWithdrawal.sub(amountToWithdraw)
        );
